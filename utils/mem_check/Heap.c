...
#define __FILE_NAME__ "Heap.c"
#endif /* __FILE_NAME__ */
#define Log(fmt, args...) printf("[MEMCHECK]%s:%d[%s()]"fmt"\n", __FILE_NAME__, __LINE__, __FUNCTION__, ##args)
...
static void checkEyecatchers(char* file, int line, void* p, size_t size)
{
	...
	if ((us = *--sp) != eyecatcher)
		Log("Invalid %s eyecatcher %d in heap item at file %s line %d", "start", us, file, line);

	cp += size;
	if ((us = *(int*)cp) != eyecatcher)
		Log("Invalid %s eyecatcher %d in heap item at file %s line %d", "end", us, file, line);
}
...
static int Internal_heap_unlink(char* file, int line, void* p)
{
    ...
    if (e == NULL)
        Log("Failed to remove heap item at file %s line %d", file, line);
    else
    {
...
void myfree(char* file, int line, void* p)
{
	...
	{
		Log("Call of free(NULL) in %s,%d",file,line);
	}
}
...
static void HeapScan()
{
	...
	Thread_lock_mutex(heap_mutex);
	Log("Heap scan start, total %d bytes", (int)state.current_size);
	while ((current = TreeNextElement(&heap, current)) != NULL)
	{
		storageElement* s = (storageElement*)(current->content);
		Log("Heap element size %d, line %d, file %s, ptr %p", (int)s->size, s->line, s->file, s->ptr);
		Log("  Content %.*s", (10 > current->size) ? (int)s->size : 10, (char*)(((int*)s->ptr) + 1));
#if defined(HEAP_STACK)
		Log("  Stack:\n%s", s->stack);
#endif
	}
...
void* mycalloc(char* file, int line, size_t nmemb, size_t size)
{
    void* mem_ptr = mymalloc(file, line, nmemb * size);
    if (mem_ptr) {
        memset(mem_ptr, 0x00, nmemb * size);
    }
    return mem_ptr;
}

/* Make Valgrind Happy */
char* mystrdup(char* file, int line, const char* src)
{
    size_t mlen = strlen(src) + 1;
    char* temp = mymalloc(file, line, mlen);
    memset(temp, '\0', mlen);
